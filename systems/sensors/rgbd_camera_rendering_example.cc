...
DEFINE_bool(show_window, true,
            ...
            "context.");
DEFINE_double(duration, .01, "Total duration of the simulation in secondes.");
...
void Generate(int num, std::ofstream& out) {
  ...
  auto rgbd_camera =
      builder.AddSystem<sensors::RgbdCameraDiscrete>(
          std::make_unique<sensors::RgbdCamera>(
              ...
              0.001, 100.0, M_PI * 50 / 180.0, FLAGS_show_window),
      kCameraUpdatePeriod);
  ...
  if (!CheckEmpty(sys_label)) {
    auto quat =
      math::RollPitchYaw<double>(Eigen::Vector3d(roll, pitch, yaw)).ToQuaternion();
    string result = "";
    ...
    for (const auto& body : plant->get_rigid_body_tree().get_bodies()) {
      ...
      SaveLabelToFile<PixelType::kLabel16I>(
        FLAGS_output_path + std::string("/images/label") + n + ".png", sys_label);

      auto sys_rgb =
        output->GetMutableData(0)->GetMutableValue<ImageRgba8U>();
      SaveToFile<PixelType::kRgba8U>(
        FLAGS_output_path + std::string("/images/rgb") + n + ".png", sys_rgb);
    }
    cout << result << endl;
    out << result;
  }
...
int main() {
  ...
  std::ofstream out(FLAGS_output_path + "/sdf/" + std::to_string(FLAGS_num) + ".yaml");
